...
export default function GoClient({ id }: { id: string }) {
  ...
  // Telegram bots redirect on every platform; App Store links only on iOS
  const targetUrl = app?.telegramUrl ?? app?.appStoreUrl ?? null
            ...
            {redirecting ? (
              <div className="space-y-4">
                ...
                <p className="text-white/90 text-lg font-medium">
                  {app.telegramUrl ? t('redirect.openingTelegram') : t('redirect.opening')}
                </p>
                <a
                  href={targetUrl}
